...
const MeteroTable = (props) => {
  ...
  const saveData = (item) => {
    ...
    if(item.NewOdo < item.OdoReading){
      setValidationModalData({
        showModal:true,
        validationMessage:<p className="odoReading">Current Odo reading is <span className="default-color">{item.OdoReading}</span><br/>
        Please enter reading greater than  <span className="default-color">{item.OdoReading}</span></p>,
        cancelButtonText:"Ok",
        showActionButton:false,
        showCancelButton:true,
        showFooterActions:true,
  ...
  return (
    ...
    showFooterActions:true
        ...
        <div className="d-flex justify-content-center align-item-center no-job-selected">
          <p>{props.selectedJob && meteroTableList.isSuccess == true ? 'No data to show' :'No Job Selected'}</p>
        </div>
      )}
    </div>
  );
